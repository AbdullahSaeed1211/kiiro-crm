...
- Repositories on the Local API: `createTaskRepository(req)` (reads and compare-and-set writes with the user's access; transitions are not stored in the spike), `createNotificationStore(payload)`, `createDueItemSource(payload)` and `createEmailMessageSink(payload)` for the Cloudflare adapter's cron and inbound ports. Payload's update-by-where finds ids and then writes by id, so a concurrent write between the two can still be lost (D-36).
