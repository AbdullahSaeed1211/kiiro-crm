...
import { byCollection, dealDoc, guard, leadDoc, MS, SCOPED_READ, setup, USER, workflowDoc } from './fixtures'
...
describe('createCrmRepository saveStage', () => {
  it('saves the stage by compare-and-set with the user access', async () => {
    ...
    expect(calls[0]?.args).toMatchObject({
      collection: 'leads',
      where: guard('l1'),
      data: { stageId: 's-won', stageEnteredAt: 9000 },
  ...
  it('returns undefined on a compare-and-set miss and for other record types', async () => {
    ...
    expect(calls[0]?.args).toMatchObject({ collection: 'deals', where: guard('d1') })
  })
})
