...
| Notification task context                                         | M3-L1 attempt 34; `pnpm verify` and scope checks pass; Worker-preview E2E passes 48/48 serially; intercepted 503 keeps the alert unread with retry feedback, then success preserves the selected month in `returnTo` and opens the task panel; no notification or CRM record was created                                              | complete |
...
| Board drag rejection guard                                        | M3-L1 attempt 35; `pnpm verify` and scope checks pass; Worker-preview E2E passes 48/48 serially; dragging a card to the board header leaves its stage unchanged and does not open task details; mobile drag stays disabled; no record content was changed                                                                             | complete |
