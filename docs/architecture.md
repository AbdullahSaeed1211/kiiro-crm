...
- **Failures are values.** Commands return `Result`; server actions and routes convert with `toActionResult`, and unexpected exceptions go through `actionFailure`, which logs the cause and returns generic copy. Some Payload adapter methods still throw for missing configuration (code-health DOM-09); `actionFailure` catches those until they return `Result`.
