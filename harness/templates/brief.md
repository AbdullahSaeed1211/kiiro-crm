...
- **Data-dependent controls:** check optional fields as null, empty, whitespace, and populated before rendering their actions or detail values. A related-record create link must carry the parent selection into the form and return Cancel to that parent. When the page title is in the content, keep the sticky nav on its parent crumb to avoid repeating the title. Verify the visible actions in a browser before closing the work package.
- **Form interaction:** submit with required fields empty, confirm inline errors, and confirm focus moves to the first invalid control. On a server error, keep the entered values and show the error beside the form instead of silently returning.
- **Pre-push formatting gate:** run `pnpm format:check` on the integrated worktree before pushing; format-check failures in newly integrated files block release verification even when feature-focused tests pass.
