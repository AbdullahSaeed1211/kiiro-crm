...
import { getContact, listOrganizations } from '../../../../../server/crm/directory/data'

export const dynamic = 'force-dynamic'

export default async function EditContactPage({ params }: Readonly<{ params: Promise<{ id: string }> }>) {
  const id = (await params).id
  const [data, organizations] = await Promise.all([getContact(id), listOrganizations()])
    ...
    <>
      ...
      <PageContent>
        ...
        <DirectoryForm
          ...
          organizations={organizations.items.map(({ record: organization }) => ({
